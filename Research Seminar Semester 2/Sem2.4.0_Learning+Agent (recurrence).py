...
def AI_loop():
	...
	#if turn away from the closest wall
    if closestWall == 1:
        ai.setTurnSpeed(64)
        ai.turnRight(1)
        turn = 1
	...
	#if we are going too fast turn around
    if speed > 2 and closestVal > 200:
        ai.setTurnSpeed(64)
        ai.turnRight(1)
        turn = 1
    ...
    elif ai.angleDiff(heading, tracking) > 120 and speed >2:
        ai.thrust(1)
        thrust = 1
...
midNode = 0
ai.start(AI_loop,["-name","guy1","-join","localhost"])  
